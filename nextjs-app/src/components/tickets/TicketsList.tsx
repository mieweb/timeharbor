    ...
    <div className="space-y-4 md:space-y-6">
      {teams.length === 0 && (
        <div role="alert" className="alert alert-warning">
          <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-5 w-5 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
          <span className="text-sm md:text-base">No teams found. You must join or create a team to manage tickets.</span>
          <div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
          <span className="text-sm md:text-base font-medium whitespace-nowrap">Select Team:</span>
          <select 
            className="select select-bordered select-sm md:select-md w-full sm:flex-1 sm:max-w-xs"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
        ...
        {filteredTickets.length === 0 && (
          <div className="col-span-full text-center py-10 opacity-50 text-sm md:text-base">
            No tickets found.
          </div>
        )}
      </div>
