...
export default function TeamsList({ teams, userId }: { teams: Team[], userId: string }) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-th-dark dark:text-white mb-2">Teams</h1>
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        <button 
          ...
          className="px-6 py-2.5 bg-th-accent hover:bg-opacity-90 text-white font-medium rounded-lg transition-colors shadow-sm"
        >
          + Create Team
        </button>
        <button 
          onClick={() => { setShowJoin(true); setShowCreate(false); setError(null) }} 
          className="px-6 py-2.5 bg-th-accent/10 hover:bg-th-accent/20 text-th-accent font-medium rounded-lg transition-colors"
        >
          Join Team
        </button>
      </div>
      ...
      {showCreate && (
        <form action={handleCreate} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8 max-w-md">
          <h4 className="font-bold text-lg mb-4 text-th-dark">Create New Team</h4>
          <div className="flex flex-col gap-4">
            <input type="text" name="name" placeholder="Team Name" className="input input-bordered w-full" required />
            <div className="flex gap-3">
              <button type="submit" className="btn bg-th-accent hover:bg-opacity-90 text-white border-none flex-1">Create</button>
      ...
      {showJoin && (
        <form action={handleJoin} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8 max-w-md">
          <h4 className="font-bold text-lg mb-4 text-th-dark">Join Existing Team</h4>
          <div className="flex flex-col gap-4">
            <input type="text" name="code" placeholder="Enter Team Code" className="input input-bordered w-full" required />
            <div className="flex gap-3">
              <button type="submit" className="btn bg-th-accent hover:bg-opacity-90 text-white border-none flex-1">Join</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {teams.map(team => (
          <div key={team.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 flex flex-col h-full hover:shadow-md transition-shadow shadow-[inset_0px_4px_0px_0px_#76ABAE]">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-bold text-th-dark dark:text-white truncate pr-2">{team.name}</h3>
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                ['leader', 'admin'].includes(team.role) 
                  ? 'bg-th-accent/10 text-th-accent dark:bg-th-accent/20 dark:text-th-accent' 
            ...
            <Link 
              ...
              className="w-full py-2.5 bg-th-accent/10 hover:bg-th-accent/20 text-th-accent dark:bg-th-accent/20 dark:hover:bg-th-accent/30 dark:text-th-accent font-medium rounded-lg text-center transition-colors"
            >
              View Team
            </Link>
          </div>
        ...
        {teams.length === 0 && !showCreate && !showJoin && (
          <div className="col-span-full text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
            ...
            <button 
              ...
              className="text-th-accent hover:text-th-accent/80 font-medium"
            >
